...
const MIN_LENGTH = 4;
export function historyFilePath(configDir) {
    return join(configDir, "autocomplete-history.jsonl");
}
export async function appendHistory(configDir, text) {
...
export async function loadHistory(configDir) {
    let raw;
    try {
 ...
 */
export function bestMatch(input, entries) {
