...
import { bestMatch, loadHistory } from "./history";
...
function PromptWithHistoryAutocomplete(props) {
    ...
    const [dismissedInput, setDismissedInput] = createSignal();
    const [history, { refetch }] = createResource(() => props.api.state.path.config, (configDir) => loadHistory(configDir), { initialValue: [] });
    const bind = (ref) => {
        setPrompt(ref);
    ...
    onMount(() => {
        ...
        const submitGuard = (evt) => {
            if (!suggestion())
                return;
            const name = evt.name?.toLowerCase();
            const isSubmit = name === "return" || name === "linefeed" || name === "enter" ||
                evt.raw === "\r" || evt.raw === "\n" ||
        ...
        props.api.renderer.keyInput.prependListener("keypress", submitGuard);
        onCleanup(() => {
            props.api.renderer.keyInput.removeListener("keypress", submitGuard);
        });
    });
    return (<box>
      <Show when={suggestion()}>
        <box position="absolute" top={-1} left={0} right={0} zIndex={100} paddingLeft={1} paddingRight={1} backgroundColor={props.api.theme.current.backgroundPanel}>
          <text fg={props.api.theme.current.textMuted} wrapMode="none">
            {suggestion().text}
          </text>
        </box>
      </Show>
      <props.api.ui.Prompt sessionID={props.sessionID} visible={props.visible} disabled={props.disabled} onSubmit={props.onSubmit} ref={bind}/>
    </box>);
}
const tui = async (api) => {
    let currentPrompt;
    ...
    api.slots.register({
        ...
        slots: {
            home_prompt(_ctx, value) {
                return (<PromptWithHistoryAutocomplete api={api} bindPrompt={bindPrompt} hostRef={value.ref}/>);
            },
            session_prompt(_ctx, value) {
                return (<PromptWithHistoryAutocomplete api={api} bindPrompt={bindPrompt} hostRef={value.ref} sessionID={value.session_id} visible={value.visible} disabled={value.disabled} onSubmit={value.on_submit}/>);
            },
        },
    });
};
